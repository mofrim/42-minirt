...
/*   Updated: 2025/05/10 19:57:42 by zrz              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	t_v3 hitpoint)
{
	...
	norm_p->incident_ray_dir = v3_get_norm(v3_minus_vec(hitpoint, \
scene.cam->pos));
	norm_p->is_cap_hit = 0;
...
t_colr	cylinder_get_colr(t_scene scene, t_objlst cylobj, t_v3 hitpoint)
{
	...
	setup_normal_params(&normal_params, scene, cylobj, hitpoint);
	surface_normal = calculate_cylinder_normal_at_hit(&normal_params);
	final_color = calculate_lights(scene, hitpoint, surface_normal, cylobj);
	return (final_color);
}
