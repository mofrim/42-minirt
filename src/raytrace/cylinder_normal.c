...
/*   Updated: 2025/05/10 20:05:38 by zrz              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** radius.
*/
static int	is_on_cap_surface_internal(t_cyl_normal_params *params, \
int cap_select)
{
	t_v3	cap_center;
	t_v3	vec_to_hit;
	...
	{
		dist_from_center_sq = v3_norm_sq(vec_to_hit);
		if (dist_from_center_sq <= params->cyl->r_squared + EPSILON)
			return (1);
	}
	return (0);
}
